    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if isinstance(self.ant, QueenAnt) and self.ant.imposter:
                    self.ant = None
                    QueenAnt.num_queens -= 1
                elif hasattr(self.ant, 'container') and self.ant.container:
                    self.ant = self.ant.ant
                elif not isinstance(self.ant, QueenAnt):
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if isinstance(self.ant.ant, QueenAnt) and self.ant.ant.imposter:
                        self.ant.ant = None
                    elif isinstance(self.ant.ant, QueenAnt) and not self.ant.ant.imposter:
                        return
                    else:
                        self.ant.ant = None
                else:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            def double_damage(place):
                if place.ant:
                    if place.ant.container:
                        if place.ant.ant:
                            if place.ant.ant not in self.doubled:
                                if place.ant not in self.doubled:
                                    ...
                                    place.ant.ant.damage *= 2
                                    self.doubled.append(place.ant.ant)
                            elif place.ant not in self.doubled:
                                place.ant.damage *= 2
                                self.doubled.append(place.ant)
                        elif place.ant not in self.doubled:
                            place.ant.damage *= 2
            ...
            place_behind = self.place
            while place_behind.exit:
                place_behind = place_behind.exit
                double_damage(place_behind)
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        Insect.reduce_armor(self, amount)
        if isinstance(self, QueenAnt) and not self.imposter and self.armor <= 0:
            bees_win()
        # END Problem 13
